...
snakemake -j 1 -n -F all

# generate DAG etc using:
snakemake -j 1 -n -F all --dag | dot -Tpdf >docs/snakemake_dag.pdf
snakemake -j 1 -n -F all --rulegraph | dot -Tpdf >docs/snakemake_rulegraph.pdf
snakemake -j 1 -n -F all --filegraph | dot -Tpdf >docs/snakemake_filegraph.pdf
